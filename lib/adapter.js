...
module.exports = (function() {
  ...
  var adapter = {
    ...
    // Raw Query with speed optimization
    prepQuery: function (connectionName, table, query, cb) {
      spawnConnection(connectionName, function __QUERY__(client, cb) {
        const name = `Q${crypto.createHmac('sha256', 'q').update(query.text).digest('hex')}`

        if (!client.__PREPARED__)
          client.__PREPARED__ = new Set()

        if (client.__PREPARED__.has(name)) {
          client.query(`execute ${name} ${query.type.length ? '('+query.values.map(val => `'${String(val).replace(/'/g, "''")}'`)+')' : ''}`, cb)        
        } else {
            client.query(`
              prepare ${name} ${query.type.length ? '('+query.type.toString()+')' : ''} as ${query.text};
              execute ${name} ${query.type.length ? '('+query.values.map(val => `'${String(val).replace(/'/g, "''")}'`)+')' : ''}
            `,
            (err, result) => {
              client.__PREPARED__.add(name)
